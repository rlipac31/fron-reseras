...
function SuccessContent() {
  ...
  const phonePayment = searchParams.get("phonePayment");
        ...
        <div className="bg-brand-black/80 rounded-xl p-5 mb-8  border-white/10 text-left space-y-4 relative">
          ...
          <div className="grid grid-cols-2 gap-4 border-t border-white/5 pt-4">
            <div>
              ...
              <p className="text-brand-white text-sm">Inicio:{inicio || "Por confirmar"}</p>
              <p className="text-brand-white text-sm">Fin: {fin || "Por confirmar"}</p>
            ...
            <div>
              ...
              <p className="text-brand-white text-sm font-mono">Precio: {user?.currency?.symbol || "$"}{precio || "0.0"}</p>
              <p className="text-brand-white text-sm font-mono">Descuento {descuento || "0.0"}%</p>
            </div>
          </div>
          <div>
            <p className="text-brand-white font-medium uppercase   text-[12px] md:text-[12px]">Fecha De Pago: {formatDate(fechaPago)}</p>
            <p className="text-brand-gold font-medium uppercase text-[12px] md:text-[12px]">Metodo De Pago: <span className="text-brand-white"> {metodoPago}</span> </p>
            {phonePayment && (
              <p className="text-brand-gold font-medium uppercase text-[12px] md:text-[12px]">
          ...
          <div className="border-t border-dashed border-white/20 pt-4 flex justify-between items-center">
            ...
            <p className="text-brand-gold font-black text-xl">{user?.currency?.symbol || "$"} {total}</p>
          </div>
        </div>
