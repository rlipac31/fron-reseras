...
import { updateBusinessConfig } from '@/app/actions/config';
...
export default function ConfigPage({ initialData }: { initialData?: any }) {
  ...
  const [formData, setFormData] = useState({
    zonaHoraria: initialData?.zonaHoraria || "America/Lima",
    currencyCode: initialData?.currency?.code || "PEN",
    currencySymbol: initialData?.currency?.symbol || "S/",
    slotDuration: initialData?.slotDuration || 60,
  });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="flex items-center gap-3 mb-8">
        <div className="bg-brand-gold p-3 rounded-2xl shadow-lg shadow-brand-gold/20">
          <Settings className="text-brand-black" size={28} />
        </div>
        <div>
          <h1 className="text-2xl font-black text-brand-black uppercase italic tracking-tighter">Ajustes del Negocio</h1>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-6">
          <div className="bg-white rounded-3xl border border-brand-gray p-8 shadow-sm">
            <h2 className="text-sm font-black text-brand-black uppercase mb-6 flex items-center gap-2">
              <Globe size={18} className="text-brand-gold" /> Localización y Horarios
            </h2>
            ...
            <div className="grid grid-cols-1 gap-6">
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  className="w-full bg-brand-gray/30 border border-brand-gray rounded-xl px-4 py-3 text-sm font-bold focus:ring-4 focus:ring-brand-gold/10 focus:border-brand-gold outline-none transition-all"
                >
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onChange={handleChange}
                    className="w-full bg-brand-gray/30 border border-brand-gray rounded-xl pl-12 pr-4 py-3 text-sm font-bold focus:ring-4 focus:ring-brand-gold/10 focus:border-brand-gold outline-none transition-all"
                  />
                </div>
              </div>
            </div>
        ...
        <div className="space-y-6">
          <div className="bg-brand-black rounded-3xl p-8 shadow-xl text-white">
            <h2 className="text-sm font-black text-brand-gold uppercase mb-6 flex items-center gap-2">
              ...
              <div className="space-y-2">
                ...
                >
                  {SOUTH_AMERICA_CURRENCIES.map(curr => (
                    <option key={curr.code} value={curr.code} className="bg-brand-black">{curr.label}</option>
                  ))}
                </select>
              </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-brand-gold text-brand-black font-black py-4 rounded-2xl uppercase tracking-tighter shadow-lg shadow-brand-gold/20 hover:scale-[1.02] active:scale-[0.98] transition-all disabled:opacity-50 flex items-center justify-center gap-2"
          >
      ...
      {showLogoutModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="absolute inset-0 bg-brand-black/80 backdrop-blur-sm animate-in fade-in duration-300" />

          {/* Contenido del Modal */}
          <div className="relative bg-white rounded-[32px] p-8 max-w-md w-full shadow-2xl border border-brand-gray animate-in zoom-in-95 duration-300">
            <div className="flex flex-col items-center text-center">
              <div className="bg-brand-gold/10 p-4 rounded-2xl mb-6">
                <LogOut className="text-brand-gold" size={40} />
              </div>

              <h3 className="text-2xl font-black text-brand-black uppercase italic tracking-tighter mb-2">
                ¡Configuración Guardada!
              </h3>
              <p className="text-gray-500 text-sm font-bold leading-relaxed mb-8 uppercase tracking-tight">
                Para que los cambios de moneda y zona horaria se apliquen en todo el sistema, es necesario <span className="text-brand-black">reiniciar tu sesión</span>.
              </p>
              ...
              <div className="grid grid-cols-2 gap-4 w-full">
                ...
                <button
                  ...
                  className="py-4 px-6 rounded-2xl text-[10px] font-black uppercase tracking-widest bg-brand-black text-brand-gold shadow-lg shadow-brand-black/20 hover:scale-[1.02] active:scale-[0.98] transition-all"
                >
                  Cerrar Sesión Ahora
                </button>
              </div>
