...
export default function BookingTicket({ booking }: { booking: any }) {
  ...
  const qrUrl = typeof window !== 'undefined'
    ? `${window.location.origin}/${user?.slug}/dashboard/pagos?filter=${encodeURIComponent(booking.ref)}`
    : `/${user?.slug}/dashboard/pagos?filter=${encodeURIComponent(booking.ref)}`;
        ...
        <div className="space-y-2">
          ...
          <div className="flex justify-between items-center">
            ...
            <span className="text-sm font-bold text-[10px] uppercase">
              Inicio {booking.inicio} - Fin {booking.fin}
            </span>
          </div>
          <div className="flex justify-between items-center">
          ...
          <div className="flex justify-between items-center">
            ...
            <span className="text-sm font-bold text-[10px] uppercase">
              {user?.currency?.symbol || "$"} {booking.precio}
            </span>
          </div>
          <div className="flex justify-between items-center">
          ...
          <div className="pt-6 border-t border-dashed border-slate-300">
            <div className="flex justify-between items-end ">
              ...
              <div className="text-right">
                ...
                <span className="text-3xl font-black">{user?.currency?.symbol || "$"} {booking.total}</span>
              </div>
            </div>
          </div>
        </div>
