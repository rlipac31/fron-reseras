...
export default function EditUserForm({ userData, businessSlug }: EditUserFormProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (isEditable.phone && formData.phone.length < 9) {
            setErrorMsg("El número debe tener entre 9 y 10 dígitos");
            return;
        }

        setLoading(true);
        // Enviamos solo los campos que están habilitados para editar (o el uid que es necesario)
        const dataToSend: any = { uid: formData.uid };
        ...
        const result = await updateUser(userData.uid, dataToSend);

        if (result.success) {
            setLoading(false);
            setShowSuccess(true);
            setTimeout(() => {
                router.push(`/${businessSlug}/dashboard/usuarios`);
                router.refresh();
            }, 1500);
        } else {
            setLoading(false);
            ...
            {showSuccess && (
                <div className="fixed inset-0 bg-brand-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-white rounded-3xl p-8 flex flex-col items-center text-center shadow-2xl animate-in zoom-in duration-300">
                        ...
                        <h3 className="text-2xl font-black text-brand-black uppercase italic">¡Usuario Actualizado!</h3>
                ...
                <div className="bg-brand-black p-6 flex items-center justify-between border-b-4 border-brand-gold">
                    <div>
                        <h2 className="text-xl font-black text-white uppercase tracking-tighter flex items-center gap-2">
                            <Save size={24} className="text-brand-gold" /> Editar Usuario (Staff)
                        </h2>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="md:col-span-2 space-y-1.5">
                            <div className="flex items-center justify-between ml-1">
                                <label className="text-[11px] font-black text-brand-black uppercase flex items-center gap-1.5">
                                    <User size={16} className="text-brand-gold" /> Nombre Completo
                                </label>
                                <button
                                    type="button"
                        ...
                        <div className="space-y-1.5">
                            <div className="flex items-center justify-between ml-1">
                                <label className="text-[11px] font-black text-brand-black uppercase flex items-center gap-1.5">
                                    <Mail size={16} className="text-brand-gold" /> Email (No editable)
                                </label>
                            </div>
                            <input
                                required
                        ...
                        {/* <div className="space-y-1.5">
                            <div className="flex items-center justify-between ml-1">
                                <label className="text-[11px] font-black text-brand-black uppercase flex items-center gap-1.5">
                                    <Shield size={16} className="text-brand-gold" /> Rol de Usuario
                                </label>
                                <button
                                    type="button"
                                    onClick={() => toggleField('role')}
                                    className={`p-1.5 rounded-lg transition-all ${isEditable.role ? 'text-brand-gold bg-brand-black shadow-lg scale-105' : 'text-gray-400 hover:text-brand-black bg-gray-100'}`}
                                >
                                    {isEditable.role ? <Unlock size={16} /> : <Lock size={16} />}
                                </button>
                            </div>
                            <select
                                name="role"
                                ...
                                className={`w-full px-4 py-3 border border-brand-gray rounded-xl text-sm outline-none transition-all font-semibold ${!isEditable.role ? 'bg-gray-100 text-gray-400 cursor-not-allowed border-dashed' : 'bg-gray-50 focus:border-brand-black focus:ring-4 focus:ring-brand-gold/20 shadow-inner'}`}
                            >
                        ...
                        <div className="space-y-1.5">
                            <div className="flex items-center justify-between ml-1">
                                <label className="text-[11px] font-black text-brand-black uppercase flex items-center gap-1.5">
                                    <Edit2 size={16} className="text-brand-gold" /> Cambiar Contraseña
                                </label>
                                <button
                                    type="button"
                                    onClick={() => toggleField('password')}
                                    className={`p-1.5 rounded-lg transition-all ${isEditable.password ? 'text-brand-gold bg-brand-black shadow-lg scale-105' : 'text-gray-400 hover:text-brand-black bg-gray-100'}`}
                                >
                                    {isEditable.password ? <Unlock size={16} /> : <Lock size={16} />}
                                    {/* {isEditable.password ? 'Edición Habilitada' : 'Desbloquear Password'} */}
                                </button>
                            </div>
                            <input
                                type='password'
                                ...
                                placeholder={isEditable.password ? 'Ingresa nueva contraseña (mín. 6 caracteres)' : '••••••••'}
                            />
                        </div>
                        ...
                        <div className="space-y-1.5">
                            <div className="flex items-center justify-between ml-1">
                                <label className="text-[11px] font-black text-brand-black uppercase flex items-center gap-1.5">
                                    <Fingerprint size={16} className="text-brand-gold" /> DNI
                                </label>
                                <button
                                    type="button"
